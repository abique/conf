...
syn match	cType		"\<\(int\|long\|short\|char\|void\)\>"
syn match	cType		"\<\(signed\|unsigned\|float\|double\)\>"
syn match	cType		"\<struct \(statfs\|passwd\|group\|stat\)\>"
syn match	cType		"\Wt_[a-z][a-zA-Z0-9_]*\W"me=e-1,ms=s+1
if !exists("c_no_ansi") || exists("c_ansi_typedefs")
  syn match   cType		"\<size_t\|wchar_t\|ptrdiff_t\|sig_atomic_t\|fpos_t\>"
  syn match   cType		"\<\(clock_t\|time_t\|va_list\|jmp_buf\|FILE\|DIR\|div_t\|ldiv_t\)\>"
...
syn match	cStruct		display "\<s_[a-z][a-z0-9_]*\>"
syn match	cUnion		display "\<u_[a-z][a-z0-9_]*\>"
syn match	cTypedef	display "\<t_[a-z][a-z0-9_]*\>"
syn match	cTypedef	display "\<[a-z][a-z0-9_]*_t\>"
syn match 	cFct		display "\w\+(."me=e-2

syn keyword	cTypedef	typedef
" A bunch of useful C keywords
syn match	cKeywordError	"\(if\|switch\|while\|for\)\s*("me=e-1
syn match	cKeywordError	"\(goto\|break\|return\|continue\|asm\|case\|default\)\s*\([^;]\|\S\)"me=e-1
syn match	cKeywordError	"sizeof\s*\S"me=e-1
syn match	cStatement	"\(goto\|break\|return\|continue\|asm\)\( \([^;]\|\S\)\|;\)"me=e-1
syn keyword	cLabel		case default
syn keyword	cConditional	else
syn match	cConditional	"\(if\|switch\) ("me=e-2
syn match	cRepeat		"\(while\|for\) ("me=e-2
syn keyword	cRepeat		do
syn match	cOperator	"sizeof \S"me=e-1
"syn match	cOperatorError	"\S\s*\(++\|--\|!\|-\|+\|\~\|&\|\*\)\s*\S"me=e-1,ms=s+1
"syn match	cOperatorError	"[^\s\*+-!&\~]\s*\(=\|\(+\|%\|-\|/\|\*\||\|&\|^\|>>\|<<\)=\?\|>\|<\|>=\|<=\|==\|!=\|||\|&&\|?\|:\)\s*[^\s\*+-!&\~]"me=e-1,ms=s+1
"syn match	cOperator	"\S \(=\|\(+\|%\|-\|/\|\*\||\|&\|^\|>>\|<<\)=\?\|>\|<\|>=\|<=\|==\|!=\|||\|&&\|?\|:\) \S"me=e-1,ms=s+1
"syn match	cOperator	"\S\(++\|--\)\+ \S"me=e-1,ms=s+1
"syn match	cOperator	"\S \(++\|--\|!\|-\|+\|\~\|&\|\*\)\+\S"me=e-1,ms=s+1
syn match	cOperator	"->"

syn keyword	cTodo		contained TODO FIXME XXX ???
syn match	cTodo		"\s_\S*_\s"
...
"  if exists("c_epita") && !exists("cxx_epita")
    syn match 	cSpaceError	display "(\s\+\|\s\+)"me=e-1
"  endif
endif
...
"  HiLink cStructure		Structure
  HiLink cStruct		Structure
  HiLink cUnion			Structure
  HiLink cEnum			Structure
  HiLink cTypedef		Structure
  HiLink cFct			Function
  HiLink cOperatorError		cError
  HiLink cKeywordError		cError
  ...
  HiLink cStructError		cError
  HiLink cUnionError		cError
  HiLink cEnumError		cError
  HiLink cFctError		cError
...
"map cf		^ofor (;;)<ESC>3h
"map cw		^owhile ()<ESC>
"map cm		^oint<TAB>main(int argc, char *argv[])<NL>{<NL>--argc;<NL>++argv;<NL>return 0;<NL>}<ESC>
"map cd		^odo<NL>{}<ESC>j$a while ();<ESC>h


let b:current_syntax = "c"


syn region Function_Folding start="{" end="}" transparent fold
syn sync fromstart
